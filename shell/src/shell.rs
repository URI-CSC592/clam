...
fn main() {
    ...
    // Stop partitioning when the Shannon entropy of the metadata is less than 0.1.
    ...
    // Print Shannon entropy of the knn labels
    println!(
        "KNN entropy: {:?}",
        utils::shannon_entropy_from_metadata(&knn_labels)
    );

    // Print Shannon entropy of the metadata
    println!(
        "Metadata entropy: {:?}",
        utils::shannon_entropy_from_metadata(metadata)
    );
}
