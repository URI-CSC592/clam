...
use abd_clam::{Cluster, PartitionCriterion, VecDataset};
use distances::Number;

/// Struct which holds the parameters for the minimum Shannon entropy partition criterion
///
/// The minimum Shannon entropy partition criterion stops partitioning a `Cluster` when the
/// Shannon entropy of the cluster is less than a given threshold.
///
/// # Arguments
///
/// - `threshold`: The threshold for the Shannon entropy
/// - `data`: The dataset containing the metadata
///
#[derive(Debug, Clone)]
pub struct MinShannonEntropy {
    pub threshold: f64,
    pub data: VecDataset<Vec<f32>, f32, String>,
}

/// Continue partitioning a `Cluster` while the Shannon entropy of the cluster is greater than a given threshold
///
/// # Arguments
///
